...
func (g GitHub) PRStatus(ctx context.Context, repoPath string, number int) (PRStatus, error) {
	...
	res, err := run.Command(ctx, repoPath, nil, g.cli, "pr", "view", strconv.Itoa(number), "--json", "reviewDecision,statusCheckRollup,comments,reviews,latestReviews,mergeStateStatus,mergeable,mergedAt")
	if err != nil {
		return PRStatus{}, err
	}
	var raw struct {
	...
	return PRStatus{ReviewDecision: raw.ReviewDecision, ChecksState: checksState(raw.StatusCheckRollup), MergeState: raw.MergeStateStatus, Mergeable: raw.Mergeable, Merged: strings.TrimSpace(raw.MergedAt) != "", FailingChecks: failingChecks(raw.StatusCheckRollup), Feedback: feedback}, nil
}
